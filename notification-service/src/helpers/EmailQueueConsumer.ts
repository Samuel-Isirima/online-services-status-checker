...
async function consumeEmailQueue(queueName) 
{
    ...
    channel.consume(queueName, async (message) => 
    {
        ...
        if (message !== null) 
        {
            ...
            else if(notification.type === "slack")
            {
                ...
                const otherDetails = notification.other_details;
                //Send to the slack queue
                await channel.sendToQueue("SLACK_SERVICE_QUEUE", Buffer.from(JSON.stringify({slack, message, otherDetails})));
            }
            else
            {
                  //Send to the slack queue
                  ...
                  const otherDetails = notification.other_details;
                  //Send to the slack queue
                  await channel.sendToQueue("SMS_SERVICE_QUEUE", Buffer.from(JSON.stringify({sms, message, otherDetails})));
            }
...
const queueName = `NOTIFICATION_SERVICE_EMAIL_QUEUE`;
consumeEmailQueue(queueName);
